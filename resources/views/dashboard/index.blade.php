...
<script>
    // Geramos as URLs base dinamicamente pelo Laravel para evitar erro 404
    const storeUrl = "{{ route('estoque.store') }}";
    const baseUrl  = "{{ url('estoque') }}"; // Isso garante que o caminho absoluto do seu servidor seja respeitado
    ...
    function fillEdit(id, titulo, autor, genero, paginas, ano, status) {
        ...
        // Usa a baseUrl gerada pelo Laravel + o ID do item
        document.getElementById('item-form').action  = `${baseUrl}/${id}`;
    ...
    function openDelete(id, titulo) {
        ...
        // Aplica a mesma correção para a URL de exclusão
